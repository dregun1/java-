...
//문제) 4,  3 5 2 7   >>>> 5 7 7 -1
//오큰수 >> 현재 수의 오른쪽의 큰 수 중 제일 왼쪽.
public class 오큰수_구하기 {
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 1; i < n; i ++) { // 예제는 n이 4 그러면 i가 1,2,3
			//스택이 비어 있지 않고 현재 수열이 스택 top 인덱스가 가리키는 수열보다 클 경우
			 while(!myStack.isEmpty() && A[myStack.peek()] <A[i]) { //peek가 가장 위 항목
				ans[myStack.pop()] = A[i];  
			}
			myStack.push(i);  //0해결하고 1집어넣고 1집어넣엇는데 해결못하고 2집어넣음 2해결하고 3임이제
		}//문제) 4,  3 5 2 7   >>>> 5 7 7 -1
		while(!myStack.empty()) {
